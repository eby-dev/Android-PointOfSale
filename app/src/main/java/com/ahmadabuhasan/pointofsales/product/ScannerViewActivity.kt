...
class ScannerViewActivity : BaseActivity(), ZXingScannerView.ResultHandler {
    ...
    override fun handleResult(rawResult: Result) {
        AddProductActivity.etProductCode.setText(rawResult.text)
        ...
        Dexter.withContext(this)
            ...
            .withListener(object : PermissionListener {
                override fun onPermissionGranted(permissionGrantedResponse: PermissionGrantedResponse) {
                    scannerView = ZXingScannerView(this@ScannerViewActivity)
                    val activity = this@ScannerViewActivity
                    activity.setContentView(activity.scannerView)
                    scannerView.startCamera()
                    scannerView.setResultHandler(this@ScannerViewActivity)
                }

                override fun onPermissionDenied(permissionDeniedResponse: PermissionDeniedResponse) {
                    if (permissionDeniedResponse.isPermanentlyDenied) {
                        Toasty.info(this@ScannerViewActivity, R.string.camera_permission, Toasty.LENGTH_SHORT).show()
                    }
                }
